...
def get_embedding():
    ...
    train_data_E = codecs.open('../../Data/ntcir_train_ns.txt', 'r', 'utf-8').readlines()
...
def process():
    ...
    ##########train_zip_C_0/1: list   expanded_train_zip_C: dict##########################################
    ...
    for time in range(times):
        ...
        train_zip_C.extend(temp)

    #train_zip_C : list 最终中文数据的量
    ...
    for line in train_data_E:
        ...
        tmp[position + abs(min_p)] = 1.
        if split_line[4]=='Y':
            train_zip_E_1.append([text_jieba, aspect_jieba, tmp, label, inx, clause_inx])
        else:
            
            train_zip_E_0.append([text_jieba, aspect_jieba, tmp, label, inx, clause_inx])
        #################################
        expanded_train_zip_E[split_line[8]].append([text_jieba, aspect_jieba, tmp, label, inx, clause_inx])
        #################################

    times = int(len(train_zip_E_0)/len(train_zip_E_1))
    for time in range(times):
        temp = []
        for i in range(len(train_zip_E_1)):
            temp.append(train_zip_E_1[i])
            temp.append(train_zip_E_0[i+time*len(train_zip_E_1)])
        train_zip_E.extend(temp)
    
    
    ##train_zip_E: list 最终英文数据的量
    #print('ch,eng:',len(train_zip_C),len(train_zip_E))  #50752 58344
    ...
    countC = 0
    countE = 0
    while bs*batch_size<len(train_zip_C): 
        tmp_dataC = train_zip_C[bs*batch_size:(bs+1)*batch_size]
        tmp_dataE = train_zip_E[bs*batch_size:(bs+1)*batch_size]
        t_C, a_C, p_C, l_C, inx_C, clause_inx_C = zip(*tmp_dataC)
        t_E, a_E, p_E, l_E, inx_E, clause_inx_E = zip(*tmp_dataE)
        countC += len(t_C)
        countE += len(t_E)
        ...
        train_final_data_C.append(list(zip(pad_t_C, list(a_C), p_C, l_C, inx_C, clause_inx_C)))
        train_final_data_E.append(list(zip(pad_t_E, list(a_E), p_E, l_E, inx_E, clause_inx_E)))
        bs += 1 
    ###最终数据都是中文数据的量 50752

    for key,value in expanded_train_zip_E.items():
        expanded_train_zip_E[key] = (value*2)
